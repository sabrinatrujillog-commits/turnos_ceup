...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    .bar{ display:flex; gap:10px; align-items:center; padding:12px 18px; background:rgba(2,43,58,0.75); backdrop-filter:blur(12px); -webkit-backdrop-filter:blur(12px); box-shadow:0 2px 12px rgba(15,45,107,0.15); border-bottom:none; flex-wrap:wrap; }
...
<body>
  ...
  <div class="bar">
    <div class="tabs">
      <button id="tabTurnos" class="tab active" onclick="setView('turnos')">🗂️ Turnos</button>
      <button id="tabBuses" class="tab" onclick="setView('buses')">🚌 Buses</button>
    </div>
    <div class="sep"></div>
    <button class="b-auto" onclick="auto()">⚡ Auto-sugerir</button>
    <button class="b-save" onclick="save()">💾 Guardar</button>
    <button class="b-reload" onclick="load()">↻ Recargar</button>
    <span class="chip">🚌 Viaje <b id="cfgViaje">–</b>m · ⏱ Margen <b id="cfgMargen">–</b>m</span>
  ...
  <div class="wrap">
    ...
    <div class="side">
      <div class="card t-grn"><h3><span class="emoji">🚌</span> Cupos por bus</h3><div id="cupBuses" class="muted">–</div></div>
      <div class="card t-red"><h3><span class="emoji">⚠️</span> Alertas (<span id="nalerts">0</span>)</h3><div id="alerts" class="muted">Sin alertas.</div></div>
    </div>
  </div>
...
function load(){ setStatus('Cargando…');
  google.script.run.withSuccessHandler(function(d){ DATA=d;
    ...
    buildRows(); render(); setStatus('Datos cargados ✔');
...
function validate(){ var byP={},list=[],turnoCount={},busCount={};
  ...
  ROWS.forEach(function(r){ var a=[]; var t=r.turno?slotById(r.turno):null; var bi=r.busIda?busBy(r.busIda):null; var br=r.busReg?busBy(r.busReg):null;
    ...
    if(t&&bi&&t.iniMin!=null&&bi.horaMin!=null){ if(bi.horaMin+DATA.config.viaje+DATA.config.margen>t.iniMin)a.push({t:'err',m:'🚌 llega tarde al turno'}); }
...
function renderTurnos(al){
  ...
  if(!order.length) return '<div class="card t-yel"><h3>🗂️ Turnos</h3><div class="muted">No hay turnos. Llena la hoja <b>CAT_TURNOS</b> y recarga.</div></div>';
  var html='';
  order.forEach(function(puesto){
    var enc=groups[puesto][0].encargado;
    html+='<div class="card t-yel"><h3><span class="emoji">🥅</span> '+puesto+(enc?' <small>👤 '+enc+'</small>':'')+'</h3>';
...
function renderBuses(al){
  ...
  function block(list,field){ var h='';
    list.forEach(function(b){ var asign=ROWS.filter(function(r){return r[field]===b.bus;});
      ...
      h+='</div></div>'; });
    return h; }
  var html='<div class="card t-org"><h3><span class="emoji">➡️</span> Buses de ida</h3>'+
    (ida.length?block(ida,'busIda'):'<div class="muted">Sin buses de ida en CAT_BUSES.</div>')+'</div>';
  html+='<div class="card t-grn"><h3><span class="emoji">⬅️</span> Buses de regreso</h3>'+
...
function renderAlerts(al){ document.getElementById('nalerts').textContent=al.list.length; var box=document.getElementById('alerts');
  if(!al.list.length){ box.innerHTML='<span class="ok pill">¡Todo en orden! ✔</span>'; return; }
  box.innerHTML=al.list.map(function(x){ var who=x.nombre?('<b>'+x.nombre+'</b>: '):''; return '<div class="alert-item '+x.t+'">'+who+x.m+'</div>'; }).join(''); }

// ---------- Servidor ----------
function save(){ setStatus('Guardando…'); google.script.run.withSuccessHandler(function(d){ DATA=d; setStatus('Guardado ✔'); }).withFailureHandler(fail).guardarAsignacion(ROWS); }

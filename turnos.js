...
<head>
  ...
  <style>
    ...
    :root{ --amarillo:#FAC012; --azul:#0844A7; --teal:#022B3A; --naranja:#F17121; --verde:#11A154; --rojo:#E61E14; --crema:#F5EAD0; --azulclaro:#2568C8; }
    *{ box-sizing:border-box; }
    ...
    .card.t-yel{ border-color:var(--azul); border-top-color:var(--azul); background:#fff; }
    .card.t-yel h3{ color:var(--azul) !important; }
    .card.t-grn{ border-color:#11A154; border-top-color:#11A154; background:#fff; }
    .card.t-grn h3{ color:#11A154 !important; }
    .card.t-red{ border-color:#c49000; border-top-color:#c49000; background:#fff; }
    ...
    .card.t-bus{ border-color:#FAC012; border-top-color:#FAC012; background:#fff; }
    ...
    .pchip{ display:inline-flex; align-items:center; gap:6px; background:#fff; border:1.5px solid var(--azul); color:var(--teal); border-radius:16px; padding:3px 6px 3px 10px; font-size:12px; font-weight:700; }
    .pchip.warn{ border-color:var(--rojo); color:var(--teal); background:#fff; }
    .pchip.warn .x{ background:rgba(230,30,20,0.15); color:var(--rojo); }
...
<body>
  ...
  <div class="wrap">
    ...
    <div class="side">
      ...
      <div class="card t-org"><h3><span class="emoji"><i data-lucide="alert-triangle" style="width:15px;height:15px;vertical-align:middle"></i></span> Alertas (<span id="nalerts">0</span>)</h3><div id="alerts" class="muted">Sin alertas.</div></div>
    </div>
  </div>
...
function render(){ var al=validate();
  ...
  renderCupos(al); renderAlerts(al); lucide.createIcons(); }
...
function puestoIcon(nombre){
  var n=(nombre||'').toLowerCase();
  if(/t.?pic|botiq|enfermer|salud/.test(n)) return 'cross';
  if(/escaneo|scan|qr|puerta|entrada/.test(n)) return 'scan-line';
  if(/mozo|box|caja|mesa/.test(n)) return 'wine';
  if(/guardarropa|vest|ropa/.test(n)) return 'shirt';
  if(/vip|acceso|acred/.test(n)) return 'star';
  if(/barra|bar|bebida/.test(n)) return 'wine';
  if(/limpieza|aseo/.test(n)) return 'sparkles';
  if(/seguridad|control/.test(n)) return 'shield';
  if(/foto|camara/.test(n)) return 'camera';
  if(/sonido|audio|dj/.test(n)) return 'music';
  return 'clipboard-list';
}
function busIcon(tipo){ return tipo==='ida' ? 'arrow-right' : 'arrow-left'; }
...
function renderTurnos(al){
  ...
  order.forEach(function(puesto){
    ...
    html+='<div class="card t-yel"><h3><i data-lucide="'+puestoIcon(puesto)+'" style="width:15px;height:15px;vertical-align:middle"></i> '+puesto+(enc?' <small><i data-lucide="user" style="width:15px;height:15px;vertical-align:middle"></i> '+enc+'</small>':'')+'</h3>';
...
function renderBuses(al){
  ...
  function block(list,field){ var h='';
    list.forEach(function(b){ var asign=ROWS.filter(function(r){return r[field]===b.bus;});
      ...
      h+='<div class="slot"><div class="hora"><i data-lucide="'+busIcon(b.tipo)+'" style="width:12px;height:12px;vertical-align:middle"></i> '+b.bus+' · '+b.hora+'</div><div class="badge '+col+'">'+c+'/'+b.capacidad+'</div><div class="people">';
      ...
      h+='</div></div>'; });
    return h; }
  var html='<div class="card t-bus"><h3><span class="emoji"><i data-lucide="arrow-right" style="width:15px;height:15px;vertical-align:middle"></i></span> Buses de ida</h3>'+
    (ida.length?block(ida,'busIda'):'<div class="muted">Sin buses de ida en CAT_BUSES.</div>')+'</div>';
  html+='<div class="card t-bus"><h3><span class="emoji"><i data-lucide="arrow-left" style="width:15px;height:15px;vertical-align:middle"></i></span> Buses de regreso</h3>'+
...
function downloadCSV(){
  ...
  var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='asignacion_wave.xls'; a.click();
}
